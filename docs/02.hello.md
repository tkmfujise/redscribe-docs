...
* The first argument `:spawn` is `key`
* The second argument `{ name: 'Alice', job: :wizard, level: 1 }` is `payload`

In Ruby, `:spwan` is a Symbol class, but when received by GDScript it becomes a StringName type.
