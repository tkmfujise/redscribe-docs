...
```
How do you feel about it? Everyone sees it differently, of course, but I personally enjoy the snappy contrast between `says` and `asks`, and the way `___?` feels like filling in a blank—it's quite satisfying.
...
As the content displayed in the dialogue grows longer, some of you may start to feel that the DSL-specific keywords like `says`, `asks` and `scene` are beginning to feel like noise.
...
It's hard to believe this is still Ruby—it almost feels like a plain-text format designed specially for dialogue systems.

Of course, someone seeing this for the first time might not immediately grasp what each operator means or how it behaves.

But compared to third-party dialogue systems, this approach offers a major advantage: you get to define the grammar yourself.
